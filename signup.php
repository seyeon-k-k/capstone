...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
    ...
    {?>
        <script> alert("아이디는 한글을 포함할 수 없습니다."); location.href="signup.html"; </script> 
    <?php
    }
    elseif (preg_match("/[ㄱ-ㅎㅏ-ㅣ가-힣]/", $userPassword)) 
    {?>
        <script> alert("비밀번호는 한글을 포함할 수 없습니다."); location.href="signup.html"; </script> 
    <?php
    }
    // Check if the username and password meet the minimum length requirements
    elseif (strlen($userID) < 6) 
    {?>
        <script> alert("아이디는 최소 6자 이상이어야 합니다."); location.href="signup.html"; </script> 
    <?php
    }
    elseif (strlen($userPassword) < 8) 
    {?>
        <script> alert("비밀번호는 최소 8자 이상이어야 합니다."); location.href="signup.html"; </script> 
    <?php
    }
    ...
    {?>
        <script> alert("비밀번호는 특수문자를 2개 이상 포함해야 합니다."); location.href="signup.html"; </script> 
    <?php
    }
    ...
    {?>
        <script> alert("비밀번호와 비밀번호 확인이 일치하지 않습니다."); location.href="signup.html"; </script> 
    <?php
    }
    else 
    {
        ...
        $sql = "INSERT INTO users (username, userid, userpassword, numofstickers, favcharacter, favsnack)
